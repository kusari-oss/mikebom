...
- **SC-001 (argo-cd testbed dep-graph completeness + accuracy — T014-Round-2-measured 2026-07-03)**: After milestone 157 ships, `mikebom sbom scan --path /tmp/argo-cd/ui --format cyclonedx-json` MUST produce **≥2000** dep-graph edges (measured as `[.dependencies[] | .dependsOn // [] | length] | add`) AND MUST emit **byte-exact-matching `dependsOn`** for ≥99% of all snapshot entries (Round-2 addition — completeness alone is insufficient; edges must also be correct). T014 Round-1 measurement 2026-07-03 recorded 2169 edges but a full 1329-entry snapshot audit revealed 1 real MISMATCH: `@babel/helper-create-class-features-plugin@7.29.3` emitted 1 edge instead of 8. Root cause: pnpm v9 `packages:` entries carry SEMVER-SPECIFIER `peerDependencies:` values (`^7.0.0`) rather than resolved versions, and the initial "inline-wins-over-snapshots" branch incorrectly treated those specifier declarations as resolved edges. Fix: branch on `lockfileVersion` — on v9, edges live EXCLUSIVELY in `snapshots:` and `packages:` sub-mappings are ignored. T014 Round-2 measurement post-fix: **3016 edges, 1328/1328 registry-resolvable packages have EXACT-MATCH `dependsOn` to lockfile snapshots (100% accuracy on emitted components; 0 false positives; 0 false negatives). 1 non-emitted canonical (`argo-ui@https://…tar.gz`) is a git-URL dep correctly excluded from components. Pre-157 baseline: 110 edges, zero non-root with dependsOn.** Specific spot-checks that MUST hold: `@actions/core@3.0.1` → `dependsOn = [@actions/exec@3.0.0, @actions/http-client@4.0.1]`; `@babel/core@7.29.6` → 15 exact deps including `@babel/parser`, `debug`, `gensync`, `json5`, `semver`.
