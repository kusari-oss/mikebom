...
- **SC-001 (argo-cd testbed dep-graph completeness — empirically-locked floor)**: After milestone 157 ships, `mikebom sbom scan --path /tmp/argo-cd/ui --format cyclonedx-json` MUST produce **at minimum ≥2500** dep-graph edges (measured as `[.dependencies[] | .dependsOn // [] | length] | add`) — a defensive floor conservatively based on 1329 non-root components × ~1.88 average deps per package (typical registry stat). **Aspirational target: ≥5000** edges (assumes typical 2-2.5 deps per package + Q1's peer + optional additions from react ecosystem's peer-dep density). T014 empirically measures the actual count and updates this SC inline to the observed floor — matching milestone-156's F1 empirical revision pattern. Whichever number T014 measures MUST be ≥2500 for the milestone to ship without further investigation. The specific expected shape for `@actions/core@3.0.1` — `dependsOn` MUST include the `dependencies:` entries `@actions/exec@3.0.0` + `@actions/http-client@4.0.1` — MUST appear in the emitted document (this specific spot-check is stable across any total-edge-count variance).
