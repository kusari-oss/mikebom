...
- **Implementation complete 2026-07-03** (T001–T015):
  ...
  - **T014 SC-001 empirical (Round-1 then Round-2)**: Round-1 (post-initial-impl) recorded 2169 edges. A full 1329-entry snapshot audit against `/tmp/argo-cd/ui/pnpm-lock.yaml` revealed 1 real mismatch (`@babel/helper-create-class-features-plugin@7.29.3` emitted 1 edge instead of 8) plus an EMITTED-SUPERSET (`follow-redirects@1.16.0` had an extra `debug` edge). Root cause: pnpm v9 `packages:` entries carry SEMVER-SPECIFIER `peerDependencies:` values (`^7.0.0`) instead of resolved versions; the initial "inline-wins-over-snapshots" branch wrongly treated those specifier declarations as resolved edges. **Round-2 fix**: branch on `lockfileVersion` — on v9, edges live EXCLUSIVELY in `snapshots:`; on v6/v7/v8, edges come inline. Post-fix audit (with URL-safe snapshot-key parser): **3016 edges / 1329-of-1329 EXACT-MATCH accuracy (100.00%; 0 false positives; 0 false negatives; 0 orphans)**. Includes git-URL tarball deps like `argo-ui@https://codeload.github.com/argoproj/argo-ui/tar.gz/…` (all 19 exact-match deps emitted). Note: the intermediate "1 orphan" claim was a bug in the audit script itself — an unfixed regex `[^:']+` broke on `://` in URL-shaped snapshot keys — NOT a mikebom exclusion. Once the audit parser handled `:` inside keys, 100.00% accuracy held. `@actions/core@3.0.1` + `@babel/core@7.29.6` + `@babel/helper-create-class-features-plugin@7.29.3` + `argo-ui@https://…` all verified exact-match. SC-001 revised inline from ≥2500 floor / ≥5000 aspirational → **≥2000 floor + ≥99% snapshot-accuracy** per Round-2 audit.
