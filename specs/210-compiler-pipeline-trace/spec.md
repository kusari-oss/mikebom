...
- **FR-006**: For each output artifact identified in the emitted SBOM (any component whose write-set intersects any file the SBOM emits as a `library`, `application`, `firmware`, or `file` component), mikebom MUST emit a per-component `mikebom:source-read-set` annotation carrying a deterministically-ordered list of `{path, sha256}` tuples for every file in that artifact's transitive read-set (excluding trace-noise paths per the Edge Cases).
...
- **FR-009**: The compiler-invocation event MUST also appear as a `compiler-invocation/v0.1` attestor entry within the witness-collection attestation format (alongside the existing `material`/`command-run`/`product`/`network-trace` entries per feature 006), so downstream consumers of witness attestations can read it without needing mikebom-native format support.
...
- **FR-016**: The trace MUST filter out a hardcoded set of trace-noise paths (system files under `/etc`, `/proc`, `/sys`, `/dev`; user cache paths under `~/.cache`, `~/.local/share`; ephemeral paths under `/tmp`, `/var/tmp`, `$TMPDIR`) from the emitted read-sets. An operator-configurable CLI flag (`--include-system-reads`) MUST bypass this filter for auditing.
