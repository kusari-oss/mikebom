...
- **Compiler cache hits (sccache, ccache, mold cache)**: when a cache serves an artifact, no compiler process runs → no read-set can be captured for that artifact. Per FR-015 (revised), MVP treats such components as `read-set-source = "unknown"` (no compiler-invocation write-set matched); explicit `"cache-hit"` detection is deferred to a follow-up milestone that hooks cache-server queries.
...
- **FR-015**: Components whose file path does NOT match any compiler invocation's write-set (per Clarifications Q1 mapping) MUST carry a `mikebom:read-set-source = "unknown"` transparency annotation and MUST OMIT the `mikebom:source-read-set` annotation for that component. The alternate value `"cache-hit"` is RESERVED for a follow-up milestone that adds explicit compiler-cache-server tracing (sccache/ccache/mold cache-hit detection). In this MVP, mikebom cannot distinguish a cache-served component from any other unmapped component; both collapse to `"unknown"`.
