...
- [ ] T017 [P] [US3] (CORRECTED 2026-06-19 — original task was based on a fabricated `LICENSE_FINGERPRINT_TABLE` design; the real milestone-131 site is a substring-matcher function, not a byte-slice table.) Identify discriminating substring patterns for each of MS-PL ("Microsoft Public License" + "Ms-PL"), LGPL-2.1 ("Lesser General Public License" + "Version 2.1"), LGPL-3.0 ("Lesser General Public License" + "Version 3"), MIT-0 ("MIT No Attribution"), EPL-1.0 ("Eclipse Public License" + "v 1.0"), EPL-2.0 ("Eclipse Public License" + "v 2.0"). No fixture files needed — patterns ship inline as `text.contains(...)` arms.

### Implementation for User Story 3 — Path A (offline fingerprint extension)

- [ ] T018 [US3] (CORRECTED 2026-06-19 — see T017 correction note.) In `mikebom-cli/src/scan_fs/package_db/nuget/pe_clr.rs::fingerprint_license` (at line 973, NOT a table), append new `if text.contains(...) return Some(<spdx-id>);` arms for the six new licenses per `data-model.md §License-enrichment dispatch (US3 Path A) §Shape (actual)`. Order matters: MIT-0 BEFORE MIT (both contain "Permission is hereby granted"); LGPL-3.0 / LGPL-2.1 BEFORE GPL-3.0 / GPL-2.0 (LGPL text contains "General Public License"); EPL-2.0 BEFORE EPL-1.0. SPDX-ID canonicalization is verified at the existing emission-site call to `SpdxExpression::try_canonical` (line 1147); the milestone-132 plan-correction PR description records the throw-away `rustc` probe that confirmed all six IDs pass.
