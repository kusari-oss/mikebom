...
- **SC-007 (unit test coverage, revised post-analyze 2026-07-04)**: The pnpm-lock + yarn-lock alias-parsing code paths MUST have at least 12 unit tests covering: (a) pnpm quoted-value alias (`react-helmet-async: '@slorber/react-helmet-async@1.3.0(peers)'`); (b) pnpm unquoted-value alias (`string-width-cjs: string-width@4.2.3`); (c) pnpm alias with scoped local-name (`@some-scope/name: @other-scope/other@2.0.0`); (d) pnpm alias with scoped aliased-name; (e) yarn v1 key-spec alias (`"@cosmograph/cosmos@^1.1.1", "@cosmograph/cosmos@npm:@cosmos.gl/graph":`); (f) yarn v1 alias where local-name is scoped; (g) yarn v1 alias where aliased-name is scoped; (h) malformed pnpm alias (FR-010 warn-and-skip); (i) malformed yarn alias (FR-010 warn-and-skip); (j) FR-007 byte-identity for the local-name value across scoped, unscoped, hyphenated, and mixed-case inputs (analyze finding A1); (k) FR-012 multi-alias emission — same canonical component reached via two distinct local-names (analyze finding A2); (l) FR-008 no-alias regression guard for both pnpm and yarn parsers. Per-format annotation-emission coverage is DELEGATED to the SC-008 integration test (T026) — which asserts the annotation appears in CDX, SPDX 2.3, AND SPDX 3 output for the same synthesized fixture. This matches milestone-127+'s established emission pattern at `builder.rs:1185-1205` which passes bare-string `extra_annotations` values through without envelope wrap (verified during /speckit-analyze 2026-07-04).
