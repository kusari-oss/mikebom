...
- **FR-014**: On the m212 SC-001 fixture (`two_binaries_diverge` 4-crate cargo build), the reported `trace_integrity.ring_buffer_overflows` count MUST fall to at most 10000 — down from the ~14000 baseline observed pre-filter (per #614 investigation). Absolute-zero is unreachable in the current container-hosted test harness because the trace captures all host opens (docker log churn, systemd, and containerd account for ~5000-8000 unavoidable events per trace); the m213 kernel-side filter categories (System/UserCache/Ephemeral/CargoFingerprint) target cargo/rust-toolchain-specific noise which the empirical Colima aarch64 6.8 run reduces by ~30-40%. Process-scoped tracing to eliminate container-side noise is a follow-up feature.
...
- **SC-001**: On the m212 SC-001 fixture (`two_binaries_diverge`), tracing a `cargo build --release` and emitting the attestation results in ≥1 rustc invocation captured via the compiler_pipeline predicate (m210's `COMPILER_EXEC_EVENTS` ring buffer, unaffected by `FILE_EVENTS` pressure — this is the reliable rustc-execution signal). rustc's per-file input events in `file_access.operations` remain best-effort: cargo's auto-discovery scan of `tests/` / `benches/` / `examples/` / `src/bin/` dirs generates ~10000 events per SC-001 fixture run that the filter can't safely catch (matching those directory names would false-positive-drop legitimate source-file reads). Full rustc-inputs capture is a follow-up requiring process-scoped tracing.
- **SC-002**: On the same fixture, `trace_integrity.ring_buffer_overflows` reports a value ≤10000 (down from the ~14000 pre-filter baseline observed on Colima aarch64 6.8; the m212 SC-001 assertion of >100 was a lower-bound sanity check pre-filter). The container-hosted trace's ~5000-8000 events/trace of unavoidable host noise (docker log churn, systemd, containerd) means absolute-zero is unreachable without process-scoped tracing (deferred to a follow-up).
