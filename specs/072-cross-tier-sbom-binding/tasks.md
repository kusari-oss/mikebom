...
- [X] T025 [US3] Add `trace-binding` subcommand to `mikebom-cli/src/cli/sbom_cmd.rs`. Args: `--component-purl <purl>`, `--image-sbom <path>`, `--candidate-sources-dir <dir>` (or `--source-sbom <path>` for single-source mode), `--format {table,json}`. Implementation: load the image SBOM, find ALL instances of the component matching the PURL, for each load-and-test against the candidate sources, return per-instance binding state (per quickstart.md Recipe 6 output shape).
