...
- [ ] T018 [US1] Add integration test at `waybill-cli/tests/scan_split_basic.rs` — first scenario: `cargo_workspace_split_emits_one_sbom_per_member`. Uses the m212 `two_binaries_diverge` fixture (4 cargo workspace members). Invokes `waybill sbom scan --path <fixture> --split --output-dir <tmp>` via `Command`. Asserts: 4 CDX files + 1 manifest exist; each SBOM's `metadata.component.purl` matches a distinct member; union of components across the 4 SBOMs equals the pre-feature single-SBOM component set (SC-004 verification).
...
T001 → T002 → T003 → T004 → T005 → T006 → T007 → T008 → T009 → T010 → T011 → T012 → T013 → T014 → T015 → T016 → T017 → T018 → T019 → T020 → T021 → T022 → T027 → T029 → T028 → T030 → T031 → T032 → T023 → T024 → T025 → T026 → T033 → T034 → T035 → T036 → T037 → T038 → T039.
