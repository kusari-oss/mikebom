...
- [ ] T028 [US5] Add an architectural comment at the top of `source_binding/mod.rs` explicitly documenting the forward-compat extension path (Bazel/Meson lands as a sibling observer file; the trait + registry stay observer-agnostic). One paragraph; not boilerplate.
...
- [ ] T033 Verify the const-growth-guard from milestone 108 still passes (it should — milestone 109 doesn't touch the bundled `FINGERPRINTS` const).
- [ ] T034 Mark all tasks T003-T031 with `[X]` in `specs/109-binary-source-purl-binding/tasks.md` with one-line completion notes for any deviations from the plan.
- [ ] T035 Run `./scripts/pre-pr.sh` clean. Open polish PR titled `docs+test: milestone 109 polish — docs cross-link + FR-014 audit extension`.
